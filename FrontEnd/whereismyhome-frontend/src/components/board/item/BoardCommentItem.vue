...
//   emit("commentModify", props.comment);
// }

// 수정 버튼 클릭 시
function commentModify() {
  document.querySelector("#comment" + props.comment.commentNo).classList.add("comment-hidden");
  document.querySelector("#regist" + props.comment.commentNo).classList.remove("comment-hidden");
}
...
<template>
  <div class="comment-item-container" :id="'comment' + comment.commentNo">
    <div>
      <p>
        ...
        <span id="like" class="text-dark">좋아요: {{ comment.like }}
          <svg :id="likeId" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" @click="likeBtnClick">
            <path
  ...
  <BoardCommentRegistItem :id="'regist' + comment.commentNo" class="comment-hidden" :comment="comment">
